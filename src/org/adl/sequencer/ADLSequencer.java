 ...
 * 
 * Internally, the <code>ADLSequencer</code> acts on a
...
SeqReportActivityStatus, Serializable
{
   ...
   /**
    * Indicates that the current nav request is jump or not.
    */
   ...
   {
      return iJumpRequest ? jump(iTarget) : choice(iTarget);
   }
         ...
         switch ( iRequest )
         {
            ...
            case SeqNavRequests.NAV_RESUMEALL:
               ...
               if ( mValidSequencing )
               {
                  ...
                  SeqActivity act = getActivity(delReq);
                  
                  if ( act != null && act.hasChildren(false) )
                  {
                     // Prepare for delivery
                     launch.mEndSession = mEndSession || mExitCourse;
   ...
   /**
    * This method is used to inform the sequencer that a 'Jump' navigation
    * request has occurred.
    * 
    * @param iTarget ID (<code>String</code>) of the target activity.
    * 
   ...
   private ADLLaunch jump(String iTarget)
   {
      ...
      if ( target != null )
      {
         ...
         if ( cur == null )
         {
            if ( _Debug )
            {
               System.out.println("  ::--> There was no current activity." +
               		" Cannot issue a start via jump.");
            }
         ...
         if ( process )
         {
            ...
            if ( valid != null )
            {
               ...
               else
               {
                  ...
                  {
                     System.out.println("  ::--> ERROR : " +
                                        "Cannot validate request. No valid jump targets.");
                  }
   ...
   private void validateRequests()
   {
      ...
      // no state change could have occurred since the last validation.
      SeqActivity cur = mSeqTree.getCurrentActivity();
   ...
   private Hashtable getJumpSet()
   {
      ...
      Hashtable jumptargets = new Hashtable();
      if ( mSeqTree != null && mSeqTree.getActivityMap() != null )
      {
         Iterator activities = mSeqTree.getActivityMap().values().iterator();
         while ( activities.hasNext() )
         {
            SeqActivity act = (SeqActivity)activities.next();
            
            // make sure it's a child node
            if ( act.getChildren(true) == null )
            {
               // now see if it's disabled or limit condition is violated
               if ( !checkActivity(act) )
               {
      ...
      else
      {
         if ( _Debug )
         {
            ...
            System.out.println("  ::--> mSeqTree null?: " + (mSeqTree == null));
            System.out.println("  ::--> ActivityMap null?: " + (mSeqTree.getActivityMap() == null));
         }
      }
   ...
   private String doTerminationRequest(String iRequest, boolean iTentative)
   {
      ...
      if ((mSeqTree.getCurrentActivity() == mSeqTree.getRoot())&& (seqReq == ADLSequencer.SEQ_RETRY) && (mSeqTree.getScopeID() != null) )
      {        
         Vector objectives = mSeqTree.getGlobalObjectives();
         
         if( objectives != null )
         {
            ADLSeqUtilities.clearGlobalObjs( mSeqTree.getLearnerID(), 
               mSeqTree.getScopeID(), objectives );
         }
         
       
      }
      
      ADLSeqUtilities.deleteDataStore((iRequest.equals(TER_ABANDONALL) || 
                                       iRequest.equals(TER_EXITALL)) &&
                                       !getRoot().getIsSuspended() && 
                                       !mSeqTree.dataScopedForAllAttempts());
   ...
   private void endAttempt(SeqActivity iTarget, boolean iTentative)
   {
      ...
      if ( iTarget != null )
      {
         ...
         if ( !iTentative )
         {
            ...
            // Invoke rollup
            invokeRollup(iTarget, getGlobalObjs(iTarget));//null);            
         }
      }
   ...
   /**
    * Returns a list of global objectives that are written to for specified 
    * Sequencing Activity
    * @param iTarget the activity for which to gather the write mapped global objectives
    * @return the list of global objective ids that are written to for the specified Activity
    */
   private Vector getGlobalObjs(SeqActivity iTarget)
   {
      Vector objs = iTarget.getObjectives();
      Vector writeMaps = new Vector();
      if ( objs != null )
      {
      ...
      protected Vector getTOC(SeqActivity iStart)
   {
      ...
      Vector toc = new Vector();  //the return vector of renderable ADLTOC objects
      ADLTOC temp = null; //used in iterations over the ADLTOC objects in the tree
      boolean done = false; //used to create the initial tree in pass 1
      ...
      int depth = 0; //used for tracking the depth of the tree
      int parentTOC = -1; //used to determine the parent of a tree
      Vector lookAt = new Vector();
      ...
      if ( walk == null )
      {
	   ...
	   // as long as there was a tree, the debug message that we are beginning can print
      if ( !done && _Debug )
	   {
            System.out.println("  ::--> Building TOC from this node:  " + walk.getID());
      }
	  
	   //if there was an activity left when the user Suspended All, bring it up
      SeqActivity cur = mSeqTree.getFirstCandidate();
      int curIdx = -1;
	
	   //if not, the current activity is what cur will become
      if ( cur == null )
      {
         ...
         if ( include )
         {
            ...
            if ( cur != null )
            {
               if ( temp.mID.equals(cur.getID()) )
               {
                  ...
                  curIdx = toc.size(); //the index is set to the last node
               }
            }
            ...
            toc.add(temp); //this node is now added with the relevant information
         }
         else
         {
            temp = new ADLTOC();
      ...
      for ( int i = 0; i < toc.size(); i++ )
      {
         ...
         if (hiddenDepth != -1)
         {
            // Check to see if we are done hiding activities, if we are outside the tree, we are done
      ...
      if ( noExit != null )
      {
         depth = -1; //depth will track the depth of the disbled node and will catch all nodes less than
      ...
      if ( mSeqTree.getFirstCandidate() != null )
      {
         ...
         if ( walk.getID().equals(mSeqTree.getRoot().getID()) )
         {
            walk = null;
         }
      }
	  ...
	  //historically here we have re-drawn depths of nodes that had certain conditions.  With the TOC now using only attributes to decide
	  //what is drawn, this code should no longer be necessary.
      ...
      //there was code here that attempted to redraw everything below the activity.  Those nodes shouldn't be hidden anymore.
   ...
   public void clearAttemptObjCompletionStatus(String iActivityID, String iObjID)
   {
      ...
      {
         System.out.println("  :: ADLSequencer --> BEGIN - " + 
                            "clearAttemptObjCompletionStatus");
         System.out.println("  ::--> Target activity: " + iActivityID);
         System.out.println("  ::--> Objective:       " + iObjID);
      }

      // Find the target activty
      SeqActivity target = getActivity(iActivityID);
    ...
    * @param iActivityID The activity id.
    * @param iObjID The objective id.
    * @param iCompletion The completion value to set.
    */
   ...
   public void clearAttemptObjProgressMeasure(String iActivityID, String iObjID)
   {
      ...
      {
         System.out.println("  :: ADLSequencer --> BEGIN - " + 
                            "clearAttemptObjProgressMeasure");
         System.out.println("  ::--> Target activity: " + iActivityID);
         System.out.println("  ::--> Objective:       " + iObjID);
      }
    ...
    * @param iActivityID The activity id.
    * @param iObjID The objective id.
    * @param iProgressMeasure The value for progress measure.
    */
    ...
    * @param iActivityID The activity id.
    * @param iObjID The objective id.
    * @param iMaxScore The value for max score.
    */
    ...
    * @param iActivityID The activity id.
    * @param iObjID The objective id.
    * @param iMinScore The value for min score.
    */
    ...
    * @param iActivityID The activity id.
    * @param iObjID The objective id.
    * @param iRawScore The value for raw score.
    */
   ...
   /**
    * Sets the attempt progress measure.
    * 
    * @param iID The id.
