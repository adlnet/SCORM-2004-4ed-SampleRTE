...
public class SCORM_2004_NAV_DMElement extends DMElement implements Serializable
{
                       ...
                       DMProcessingInfo oInfo)
   {
      ...
      if ( iArguments != null )
      {
         ...
         else
         {
            if ( mDescription.mBinding.equals("choice") || 
                 mDescription.mBinding.equals("jump") )
            {               
      ...
      else if ( mParent.getDMElementBindingString().equals("data") )
      {
         result = doGeneralGet(iAdmin, iDelimiters, oInfo);
         done = DMErrorCodes.NO_ERROR == result;
      }

      if ( ! done && (result == DMErrorCodes.NO_ERROR  ||
           result == DMErrorCodes.SPM_EXCEEDED) )
      {
         if ( mDescription.mBinding.equals("request") )
         {
            ...
            if ( mValue.equals("choice") || mValue.equals("jump") )
            {                 
               if ( mDelimiters != null )
               {
            ...
            else
            {
               ...
               if ( mValue != null )
               {
                  ...
                  if ( mDM.mNavRequests != null )
                  {
                     ...
                     else if (mDescription.mBinding.equals("jump"))
                     {
                        if ( mDM != null && mDM.mNavRequests.mJump != null )
                        {
                           boolean ok = mDM.mNavRequests.mJump.containsKey(target);
   ...
   public int processRequest(DMRequest ioRequest, DMProcessingInfo oInfo)
   {
      ...
      if ( ioRequest.hasMoreTokens() )
      {
         ...
         if ( tok.getType() == RequestToken.TOKEN_ELEMENT )
         {
            ...
            {
               // gettin fancy so that we can set the access based on the manifest
               RequestToken val = ioRequest.getNextToken();
               if ( val.getType() == RequestToken.TOKEN_VALUE && ioRequest.isAdminRequest() )
               {
                  String[] value = val.getValue().split("<>");
                  if ( value.length == 2 )
                  {
                     // had to make a copy of the description so that each
                     // store had separate attributes 
                     Vector dep = mDescription.mDependentOn;
                     mDescription = new DMElementDescriptor(mDescription.mBinding, null, mDescription.mSPM, 
                           mDescription.mValidator);
                     
                     mDescription.mDependentOn = dep;
                     mDescription.mIsReadable = Boolean.valueOf(value[0]).booleanValue();
                     mDescription.mIsWriteable = Boolean.valueOf(value[1]).booleanValue();
                     oInfo.mValue = "_access";
                  }
                  else
                  {
                     // value wasn't formatted right
         ...
         else if ( tok.getType() == RequestToken.TOKEN_INDEX )
         {
            ...
            else // SetValue()
            {
               ...
               if ( idx >= mRecords.size() )
               {
                  ...
                  if ( idx >= mRecords.size() && 
                        ( mDescription.mBinding.equals("data") && ! ioRequest.isAdminRequest() ) )
                  {
                     // you can't make a new data record
                     ok = false;
                     result = DMErrorCodes.GEN_SET_FAILURE;
                  }
                  else if ( idx == mRecords.size() &&
                         ...
                         mRecords.size() == mDescription.mSPM) )
                  {
                     ...
                     if ( lookAt != null )
                     {
                        ...
                        for ( int i = 0; i < mDescription.mChildren.size(); i++ )
                        {
                           desc = (DMElementDescriptor) mDescription.mChildren.elementAt(i);
                     ...
                     if ( ok )
                     {
                        ...
                        { // Create the new record
                           ...
                           DMElement element = new SCORM_2004_NAV_DMElement(desc, this, mDM);
   ...
   public int setValue(RequestToken iValue, boolean iAdmin)
   {
      ...
      if ( result == DMErrorCodes.NO_ERROR ||
           result == DMErrorCodes.SPM_EXCEEDED )
      {
         if ( mDescription.mBinding.equals("store") && iValue.getValue().equals("") )
         {
            mInitialized = false;
         ...
         else
         {
               ...
               for ( ; i < iValue.getDelimiterCount() && found; i++ )
               {
                  ...
                  for ( int j = 0; j < mDelimiters.size(); j++ )
                  {
                     ...
                     if ( toSet.mDescription.mName.equals(del.getName()) )
                     {
                        ...
                        boolean setAlready = ((Boolean) set.elementAt(j)).booleanValue();
            ...
            if ( mDescription.mBinding.equals("request") )
            {
               ...
               else if ( mValue.equals("jump") )
               {
                  mDM.mCurRequest = mValue;
                  mDM.mJumpLocation = (target.mValue);
               }
               else
               {
                  mDM.mCurRequest = mValue;
