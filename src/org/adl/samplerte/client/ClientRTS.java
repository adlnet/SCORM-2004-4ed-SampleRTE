 ...
 * be implemented to be conformant with the communication mechanisms described
 ...
 * This implementation is intended to be used by the SCORM Version 2004 4th Edition.
 * Sample RTE. <br> <br>
 *
...
public class ClientRTS extends Applet implements SCORM13APIInterface
{
   ...
   public void init()
   {
      ...
      // to the LMS.
setBackground(new Color(54,60,84));
      if( _Debug )
      {
         System.out.println("In API::init()(the applet Init method)");
         //System.out.println("registering applet APIAdapter with 'this' ?browser?...");
      }      
    ...
    * It is used by SCORM 2004 4th Edition SCOs.  The LMSCMIServlet is contacted
   ...
   public String Initialize(String iParam)
   {
      ...
      if( mTerminatedState )
      {
         if(_Debug){ System.out.println("mTerminatedState = " + mTerminatedState);}
      ...
      if( logging_on )
      {
         if(_Debug){ System.out.println("logging_on = " + logging_on);}
      ...
      if(_Debug){ System.out.println("result to return is: " + result);}
      return result;
   }
    ...
    * by a SCORM Version 2004 4th Edition SCO when the SCO has determined that it no longer
   ...
   public String Terminate(String iParam)
   {
      ...
      {
         // We have to check here first to make sure no user UI nav requests were made
         // If there was one, it will override anything the SCO set
         if ( mLMSSuspendAllPushed || mNextButtonPushed || mPreviousButtonPushed ||
               mQuitButtonPushed || mTOCPushed )
         {
            int err = DMInterface.processSetValue("adl.nav.request", mUserNavRequest, 
                  true, mSCOData);
         }
         ...
         else
         {
            ...
            else
            {
               ...
               { // SeqNavRequests.NAV_CONTINUE
                  ...
                  // SeqNavRequests.NAV_JUMP
                  else if ( event.equals("10") )
                  {
                     tempEvent = ((SCORM_2004_NAV_DM)mSCOData.getDataModel("adl")).getJumpLocation();
                     isJump = true;
                  }
                  else
                  {
            ...
            if (  !mLMSSuspendAllPushed && !mPreviousButtonPushed && !mNextButtonPushed && !mTOCPushed)
            {
    ...
    * for a SCORM 2004 4th Edition SCO.  The values are locally cached except for
    ...
    * Used by SCORM 2004 4th Edition SCOs.  The values are locally cached
    ...
    * whichever occurred most recently.  Used by SCORM 2004 3rd Edition and later SCOs.
    *
    * @param iParam ("") - empty characterstring.  An empty characterstring
...
//            request.mActivityData = mSCOData;
  ...
  /**
    ...
    * API Instance.  Used by SCORM 2004 4th Edition SCOs.
    *
    ...
    * simply returns the requested information.  Used by SCORM 2004 4th 
    * Edition SCOs.
    *
   ...
   {      
      if ( isInitialized() )
      {
         ...
         mUserNavRequest = "suspendAll";
         Commit("");
      }      
   }
   
   /**
    * This method indicates that the quit button was pushed.
    */
   public void quitButtonPushed()
   {      
      if ( isInitialized() )
      {
         mQuitButtonPushed = true;
         mUserNavRequest = "_none_";
         Commit("");
         int dmErrorCode = 0;
   ...
   /**
    * This method indicates that the previous button was pushed.
    */
   public void previousButtonPushed()
   {      
      if ( isInitialized() )
      {
         mPreviousButtonPushed = true;
         mUserNavRequest = "previous";
         int dmErrorCode = 0;
         dmErrorCode = DMInterface.processSetValue("adl.nav.request", "_none_", true, mSCOData);         
      }      
   }
   ...
   {      
      if ( isInitialized() )
      {
         mNextButtonPushed = true;
         mUserNavRequest = "continue";
         int dmErrorCode = 0;
         dmErrorCode = DMInterface.processSetValue("adl.nav.request", "_none_", true, mSCOData);         
      }      
   }
